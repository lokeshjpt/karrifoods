...
async function resolvePlugin(config, name) {
    try {
        const rootPath = path_1.resolve(config.app.rootDir, 'node_modules', name);
...
function printPlugins(plugins, platform, type = 'capacitor') {
    ...
    for (let p of plugins) {
        common_1.log(`    ${chalk.bold(`${p.name}`)} (${chalk.green(p.version)})`);
    }
}
...
function getFilePath(config, plugin, path) {
    if (path.startsWith("node_modules")) {
        return path_1.join(config.app.rootDir, path);
    }
