...
async function checkCocoaPods(config) {
    config;
    if (!await common_1.isInstalled('pod')) {
...
async function getIOSPlugins(config, allPlugins) {
...
async function resolvePlugin(config, plugin) {
    let iosPath = '';
    ...
    else if (plugin.xml && plugin_1.getPluginPlatform(plugin, 'ios')) {
        iosPath = 'src/ios';
    }
    else {
        return null;
    ...
    try {
        ...
        if (cordova_1.getIncompatibleCordovaPlugins().includes(plugin.id)) {
            plugin.ios.type = 2 /* Incompatible */;
        }
        else if (plugin.xml) {
            plugin.ios.type = 1 /* Cordova */;
        }
    }
    catch (e) {
...
async function editProjectSettingsIOS(config) {
    ...
    pbxContent = pbxContent.replace(/PRODUCT_BUNDLE_IDENTIFIER = ([^;]+)/, `PRODUCT_BUNDLE_IDENTIFIER = ${appId}`);
